...
Other org files are used for publishing only and do not contribute to the agenda. See 16 for more details.
...
on this laptop does not include the ~/git/client2 directory.
...
* Refile tasks to empty the list
  ...
  o Repeat (or refile individually with C-c C-w) until all refile tasks are gone
...
are no longer important and don't deserve my attention today.
...
fast as I can and actually do work on it - not spend time hunting through my org files for the task that needs to be clocked-in.
...
left before I take off for lunch.
...
* Punch in (start the clock)
  ...
  o This clocks in a predefined task by org-id that is the default task to clock in whenever the clock normally stops

* Clock in tasks normally, and let moving to a DONE state clock out

  o clocking out automatically clocks time on a parent task or moves back to the predefined default task if no parent exists.
...
If I am working on some task, then I simply clock in on the task. Clocking out moves the clock up to a parent task with a todo keyword (if any) which keeps the clock time in the same subtree. If there is no parent task with a todo keyword then the clock moves back to the default clocking task until I punch out or clock in some other task. When an interruption occurs I start a capture task which
...
The following tags are automatically added or removed by todo state triggers described previously in ToDo_state_triggers
...
that contributes to my agenda (ie. in org-agenda-files) then I can find the note back easily with my notes agenda view by hitting the key combination F12 N. I'm free to limit the agenda view of notes using standard agenda tag filtering.
...
clocks in my default task again (if any).
...
Most of my day is deadline/schedule driven. I work off of the agenda first and then pick items from the todo lists as outlined in 8.2
